def solution(a, b, g, s, w, t):
    ...
    while end>=start:
        ...
        for i in range(len(t)):
            ...
            # 운반 횟수 = 주어진 시간을 왕복 걸리는 시간으로 나눔
            if mid%(now_t*2) >= now_t:
            # 근데 나머지가 편도 걸리는 시간보다 크면 편도로 한번 더 갈 수 있으므로 운반을 한번 더 할 수 있음
                move_cnt += 1 
        ...
        if gold >= a and silver >= b and add >=a+b:
            # 만약에 a보다 주어진 시간내에 옮길 수 있는 금의 양이 많고,
            # b보다 ..., a+b보다 ... 세 가지 조건을 모두 만족하면 시간을 줄여서 최소값을 찾고
            end = mid - 1
